...
interface GetFollowersControllerParams {
  socialService: SocialService
}

export function getFollowersController(params: GetFollowersControllerParams): RequestHandler {
  return async (req, res) => {
    const {page, pageSize, followedId} = req.params;

    const formattedPage = parseInt(page, 10);
    const formattedPageSize = parseInt(pageSize, 10);

    const result = await params.socialService.getPaginatedFollowingRelationshipByFollowedId(followedId, formattedPage, formattedPageSize);
